...
void MultiLepEventSelector::BeginJob( const edm::ParameterSet& iConfig, edm::ConsumesCollector && iC)
{
    ...
    rhoJetsNCToken      = iC.consumes<double>(selectorConfig.getParameter<edm::InputTag>("rhoJetsNCInputTag"));
...
bool MultiLepEventSelector::METfilter(edm::Event const & event)
{
	...
	if (considerCut("MET filters")) {
	  ...
	  for (unsigned int i=0; i<PatTriggerResults->size(); i++){
	    ...
	    if (patTrigNames.triggerName(i) == "Flag_BadPFMuonDzFilter") badpfmuondzpass = PatTriggerResults->accept(patTrigNames.triggerIndex(patTrigNames.triggerName(i)));
...
void MultiLepEventSelector::MuonSelection(edm::Event const & event)
{
	...
	//packed pf candidates and rho source needed for miniIso
